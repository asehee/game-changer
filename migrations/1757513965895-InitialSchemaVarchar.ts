...
export class InitialSchemaVarchar1757513965895 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`games\` (\`id\` varchar(36) NOT NULL, \`title\` varchar(255) NOT NULL, \`version\` varchar(50) NOT NULL, \`isActive\` tinyint NOT NULL DEFAULT 1, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), INDEX \`idx_games_title\` (\`title\`), INDEX \`idx_games_is_active\` (\`isActive\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
